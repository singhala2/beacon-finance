...
| 7D | Sentry error monitoring | `@sentry/nextjs` wired for server, client, edge runtimes. User id tagged on capture; PII (emails, account numbers, access tokens) scrubbed from breadcrumbs and request bodies. One deliberate test error confirmed visible in the Sentry dashboard. Source-map upload deferred (no `SENTRY_AUTH_TOKEN`). | ✅ done |
| 7E | Legal: privacy policy + ToS + clickwrap | See expanded breakdown below. Ships as a single sub-milestone but several files. | ✅ done |
...
| 7G | `/.well-known/security.txt` | Static file under `public/.well-known/security.txt` per RFC 9116: contact, preferred languages, expiration. Cheap, expected for fintech. | ✅ done |
...
### 7D notes
`@sentry/nextjs` v10.56. Configs: `sentry.server.config.ts`, `sentry.edge.config.ts`, `instrumentation-client.ts`. `instrumentation.ts` loads server config under `nodejs` runtime and edge config under `edge`. `next.config.ts` wrapped in `withSentryConfig`; source-map upload disabled (auth token deferred). `lib/sentry-scrub.ts` redacts keys matching `email|accessToken|public_token|password|token|authorization|cookie|mask|encryption_key|plaid_secret` and email-pattern strings; wired into `beforeSend` on all three runtimes. `Sentry.setUser({id})` set in NextAuth session callback so events carry user id. `captureRequestError` re-exported from instrumentation for unhandled route-handler errors. Verified with `SENTRY_DEBUG=1` + a deliberate `/api/sentry-test` throw: server log shows `Captured error event` → `Flushing events...` → `Done flushing events`. Test route deleted post-verification.

### 7E notes
Schema: `User.acceptedTermsAt`, `User.acceptedTermsVersion`. `lib/terms.ts` exports `TERMS_VERSION = '2026-06-04'`, `SUPPORT_EMAIL`, and a `termsAreStale()` helper. Content pages live under `app/(public)/{privacy,terms}/page.tsx` with a shared `LegalShell` + `LegalSection` + `DraftWatermark` in `components/legal/`. Each page carries a clear DRAFT banner. Signin form now requires an explicit clickwrap checkbox (button disabled until checked); on successful magic-link signin the `events.signIn` callback in `lib/auth.ts` stamps the user row with `acceptedTermsAt` and the current `TERMS_VERSION`. `CookieBanner` is mounted in the root layout, dismiss persisted in localStorage. `Footer` (Privacy · Terms · Contact) added to auth layout and DashboardShell. Plaid disclosure paragraph added to `PlaidConnectCard` linking Plaid's End User Privacy Policy. Public routes (`/privacy`, `/terms`) added to middleware allowlist. **Stale-version re-prompt not yet wired** — the schema and helper are in place, but no UI redirects existing users with a stale `acceptedTermsVersion`. Adding it is a small follow-up: a check in `(app)/layout.tsx` redirecting to a `/terms/accept` route that POSTs the acceptance and continues. Flagged in KNOWN_ISSUES.
...
### 7G notes
Contact `saaanant@gmail.com`, expires 2027-06-04 (renew annually per RFC 9116). Served at `/.well-known/security.txt` automatically via Next's `public/` directory.
